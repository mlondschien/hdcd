...
#' @inheritParams hdcd 
#' @export
classifier_best_split_function <- function(classifier, optimizer, control){
  ...
  function(x, start, end, split_candidates, lambda = 0){
    ...
    gain_function <- classifier_gain_function(x = x, start = start, end = end, classifier = classifier, control = control)
    n <- nrow(x)
    # Apply optimizer           
    if(optimizer == 'section_search'){
      res <- section_search(gain_function = function(x) gain_function(x)$max_gain, 
      ...
      res <- line_search(gain_function = function(x) gain_function(x)$max_gain, 
                         split_candidates = split_candidates, n = n, control = control)
    } else if (optimizer == 'two_step_search') {
      res <- two_step_search(gain_function = gain_function, 
                             split_candidates = split_candidates, n = n, control = control)
    } 
...
kNN_best_split_function <- function(x, control){
  ...
  function(x, start, end, split_candidates, lambda = 0){
    ...
    predictions <- (n - 1) * ((predictions / c(1, 1 : (n - 1)) * !upper.tri(predictions)) + ((1 - predictions) / c((n - 1) : 1, 1) * upper.tri(predictions)))
    gain <- rep(NA, nrow(x))
    gain[ (start + 1) : end] <- rowSums(log_eps(predictions)) / nrow(x)
    best_split <- split_candidates[which.max(gain[split_candidates])]
    ...
    for (i in 1 : n_perm){
      ...
      predictions_perm <- (n - 1) * ((predictions_perm / c(1, 1 : (n - 1)) * !upper.tri(predictions_perm)) + ((1 - predictions_perm) / c((n - 1) : 1, 1) * upper.tri(predictions_perm)))
      permutation_test[i] <- max(rowSums(log_eps(predictions_perm))[split_candidates - start]) / nrow(x)
    }
...
best_split_function_from_gain_function <- function(gain_function, optimizer, control){
  
  stopifnot(optimizer %in% c('section_search', 'smooth_section_search', 'line_search', 'two_step_search', 'two_step_search_shift_in_mean', 'two_step_search_adjusted'))
  ...
  function(x, start, end, split_candidates, lambda = 0){
    ...
    gain_function <- gain_function(x = x, start = start, end = end, lambda = lambda, control = control)
    
    # Apply optimizer           
    if(optimizer == 'section_search'){
      res <- section_search(function(x) gain_function(x)$max_gain, split_candidates, n, control)
    } else if (optimizer == 'line_search'){
      res <- line_search(function(x) gain_function(x)$max_gain, split_candidates, n, control)
    } else if (optimizer == 'two_step_search') {
