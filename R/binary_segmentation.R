...
#' entries gain, max_gain, best_split and possibly permutation_test and pval.
#' @param cross_validation_function A function with arguments \code{x}, \code{start}, \code{end}, \code{lambda}, \code{folds}, \code{control}
#' @inheritParams hdcd
#' @import data.table
#' @import data.tree
binary_segmentation <- function(x, get_best_split, delta, lambda = 0, gamma = 0, segmentation = 'BS', cross_validation_function = NULL, control = hdcd_control()){
  ...
  # get a data.frame of start - end pairs for which gains and optmial splits will be calculated. 
  # Only relevant for SBS or WBS, will be NULL for BS
  segments <- data.table::as.data.table(draw_segments(n = n, n_segments = control$wbs_n_segments, delta = delta, segmentation = segmentation, alpha = control$sbs_alpha))
  
  # add column with lists of split candidates
  segments$split_candidates <- list(mapply(function(start, end){ list( (start + minimal_segment_length) : (end - minimal_segment_length) ) }, segments$start, segments$end))
  
  # calculate gain, best split, ... for all segments
  gains <- mapply(get_best_split, start = segments$start, end = segments$end,
  ...
  tree <- data.tree::Node$new(paste('(', 0, ' ', n, ']', sep = ''), start = 0, end = n)
  ...
  if(!is.null(cross_validation_function)){
    tree$folds <- sample_folds(tree$n, control$glasso_cv_inner_n_folds, randomize = control$glasso_cv_inner_randomize_folds)
    temp <- cross_validation_function(x, start = 0, end = n, lambda = lambda, folds = tree$folds, control = control)
  ...
  binary_segmentation_recursive <- function(node){
    ...
    if(!any(segments$start == node$start & segments$end == node$end)){
    ...
    best_segment <- segments[start >= node$start & end <= node$end, ][unlist(max_gain) == max(unlist(max_gain))][1]
    ...
    if(control$permutation_test){
      node$permutation_test <- apply(matrix(unlist(segments[start >= node$start & end <= node$end, permutation_test]), nrow = control$permutation_test_n), 1, max)
      node$pvalue <- (1 + sum(node$permutation_test > node$max_gain)) / (1 + length(node$permutation_test))
      if(node$pvalue >= control$permutation_test_pvalue){return(NA)}
    }
    ...
    stopifnot(node$start < node$split_point & node$split_point < node$end)
    ...
    if(!is.null(cross_validation_function)){
      temp_left <- cross_validation_function(x, start = node$start, end = node$split_point, lambda = node$lambda, folds = node$root$folds, control = control)
      temp_right <- cross_validation_function(x, start = node$split_point, end = node$end, lambda = node$lambda, folds = node$root$folds, control = control)
