...
classifier_gain_function <- function(x, start, end, classifier, control){
  function(split_point, split_candidates = split_point){
    ...
    likelihood_difference <- likelihood_matrix[, 1] - likelihood_matrix[, 2]
    
    gain <- array(NA, nrow(x))
    gain[(start + 1) : end] <- (likelihood_sum + cumsum(likelihood_difference)) / nrow(x)
    ...
    for (i in 1 : n_perm){
      ...
      permutation_test[i] <- (likelihood_sum + max(cumsum(likelihood_difference[sigma])[split_candidates - start])) / nrow(x)
    }
...
glasso_gain_function <- function(x, start, end, lambda, control = hdcd_control()) {
  ...
  function(split_point, split_candidates = split_point){
    ...
    gain[(start + 1) : end] <- (loglik_global - cumsum_right[length(cumsum_right)] - cumsum_left + cumsum_right) / nrow(x)
    best_split <- split_candidates[which.max(gain[split_candidates])]
    max_gain <- gain[best_split]
...
kNN_gain_function <- function(x, control = hdcd_control()){
  ...
  distance_matrix <- as.matrix(stats::dist(x)) # calculate pairwise distances of all observations
