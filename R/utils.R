...
compare_change_points <- function(cpts_a, cpts_b, n) {
  if (! requireNamespace("clues", quietly = TRUE)) {
    ...
    cpts_b <- setdiff(sort(cpts_b)[!duplicated(sort(cpts_b))], c(0, n))
   ...
   out <- c(clues::adjustedRand(MarkGroupings(cpts_a), MarkGroupings(cpts_b)),
...
max_min_distance <- function(cpts_a, cpts_b, n){
  if(length(cpts_b) == 0 & length(cpts_a) > 0){
    ...
    max(sapply(cpts_a, function(i) min(sapply(cpts_b, function(j) abs(i - j)))))
  }
}
...
draw_segments <- function(n, n_segments = n, delta = 0.1, segmentation = 'BS', alpha = 1/2){
  ...
  if(segmentation == 'BS'){
    ...
    for(i in 1 : k){
      ...
      df <- rbind(df,
                  data.frame(start = round(seq(from = 0, to = n - length_interval, length.out = n_intervals)),
                             end = round(seq(from = length_interval, to = n, length.out = n_intervals)))
      )
    }
    return(df)
...
train_test_split <- function(x, alpha, train_fold = 1 : nrow(x)){
  ...
  if(alpha[1] != 0) alpha <- c(0, alpha)
  if(alpha[length(alpha)] != length(train_fold)) alpha <- c(alpha, length(train_fold))
  ...
  y[train_fold[length(train_fold)] : nrow(x)] <- y[train_fold[length(train_fold)]]
