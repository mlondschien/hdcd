### Simulate
# this is the main file used for simulation on external cores.
#
# The file should be run with additional parameters.
#
# comment:

args <- commandArgs(FALSE)
#args <- c(0,0,0,'test', 500, 100, 'ChainNetwork', 'loh', 'section', 'blockwise', '0.2', '2', '1', '4', '70', '120', '120', '190')
...
# load library
library('rfcd', lib.loc = '../R/x86_64-slackware-linux-gnu-library/')
library(data.table)
...
for (i in 1 : nrep){
  ...
  x <- rfcd::simulate_from_model(rfcd::create_model(n, p, alpha, Network_fun))

  # delete values from training dataset
  x_del <- rfcd::delete_values(x, delete_fraction, deletion)
  
  tree <- rfcd::hdcd(x_del, method = 'glasso', lambda = 0.1, delta = 0.1, optimizer = optimizer,
             control = rfcd::hdcd_control(cv_inner = T, cv_inner_search_lambda = T, segment_loss_min_points = 5, section_search_min_points = 5, glasso_NA_method = NA_method))
  
  # save the true change points in a list
  true_cpts <- list(alpha)
  # extract the found change points from the tree
  found_cpts <- list(rfcd::get_change_points_from_tree(tree, variable = 'cv_improvement'))
  
  # get adj. Rand Index and other performance measures
  performance_measure <- rfcd::compare_change_points(alpha, found_cpts[[1]], n)
